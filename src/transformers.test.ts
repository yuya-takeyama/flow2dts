...
describe('transformers', () => {
  describe('transform', () => {
    const examples = [
      ...
      {
        input:
          "export default function foo(bar: string): string { reeturn 'FOO!'; }\n",
      ...
      {
        input:
          "export default function(config: Config = {}): string { return 'foo'; }\n",
        output: 'export default function(config: Config): string;\n',
      },
      {
        input:
          'export default function(): Promise<Object> { return new Promise(foo()); }\n',
      ...
      {
        input:
          'export default function persistReducer<State: Object, Action: Object>(config: PersistConfig, baseReducer: (State | void, Action) => State): (State, Action) => State & PersistPartial {}\n',
        output:
          'export default function persistReducer<State: object, Action: object>(config: PersistConfig, baseReducer: (State | void, Action) => State): (State, Action) => State & PersistPartial;\n',
      },
      ...
      {
        input:
          "import type { Foo as foo, Baz as baz, Qux as qux } from 'bar';\n",
